...
function CocktailsByIngredients({ selectedIngredient, navigation }) {
    ...
    useEffect(() => {
        if(selectedIngredient && selectedIngredient.length > 0){
            ...
            // Promise.all pour attendre que tout les promises sont réalisées
        ...
        <View style={styles.ingredientContainer}>
            {cocktails ? (
                <FlatList
                    ...
                    numColumns={1}
                    renderItem={({ item }) => (
                        <TouchableOpacity style={styles.containerItem} onPress={() => navigation.navigate('Cocktails', { screen: 'Informations Cocktail', params: { cocktailId: item.id } })}>
