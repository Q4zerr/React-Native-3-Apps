...
function App() {
  ...
  const screenOptionsTab = ({ route }) => ({
    ...
    headerLeft: () => (
      <Image 
        source={require('./assets/AppCocktails.png')} 
        style={{ width: 24, height: 24, marginLeft: 30, borderRadius: 4 }}
      />
    ),
    headerRight: () => (
        <Text style={{ marginRight: 30, color: '#404040', fontSize: 18, fontWeight: '500'}}>
          {route.name}
        </Text>
    ),
